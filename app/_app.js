	...
	},
	{
		...
		handler : function(ev){
			...
			switch(ev.type){
				case "DOMNodeRemoved":
					//if(this.inFrame(target)){
						this.element.trigger("addEvent",["removed",target, undefined, target.prettySelector()]);
					//}
				...
				case 'DOMNodeInserted':
					...
					//}
					this.element.trigger("addEvent",["added",target, undefined, , target.prettySelector()]);
					break;
				...
				case 'mouseover' : 
					...
					break;
				case 'mouseout' : 
					if(target && target.compare($(ev.relatedTarget)) != 20){
				...
				case 'keydown' : 
					...
					if(addImmediately){
						this.element.trigger("addEvent",["char", ev.target, key]);
						...
						this.keyDownTimeout = setTimeout(function(){
							...
							if($.inArray(key, controller.downKeys) == -1){
								...
								controller.element.trigger("addEvent",["char", ev.target, key])
							}
						}, 20);
					}
					break;
				...
				case 'keyup':
					...
					if(Syn.key.isSpecial(ev.keyCode)){
						...
						this.element.trigger("addEvent",["char", ev.target, "[" +key+"-up]"])
					}
				...
				case 'mouseup':
					...
					if(this.isScrolling){
						if(this.scroll != null){
							...
							this.element.trigger("addEvent",["scroll", this.scroll.target, direction, amount, this._selector]);
						}
						...
						if(/option/i.test(ev.target.nodeName)){
							...
							this.element.trigger("addEvent", ['rightClick', ev.target, undefined]);
						}else if(!this.mousemoves || (this.lastX == ev.pageX && this.lastY == ev.pageY)){
							if(this.clickTimeout){
								clearTimeout(this.clickTimeout);
								delete this.clickTimeout;
								this.element.trigger("addEvent",["doubleClick", ev.target,undefined]);
							} else {
								var controller = this,
									target = ev.target
									//prettySel = $(target).prettySelector();
								this.clickTimeout = setTimeout(function(){
									controller.element.trigger("addEvent",["click",target, undefined]);
									delete controller.clickTimeout;
								}, 200);
							}
						}else if(this.mousemoves > 2 && this.mousedownEl){
							this.element.trigger("addEvent",["drag", this.mousedownEl, {clientX : ev.clientX,
								clientY: ev.clientY}])
						}
				...
				case 'change':
					if(ev.target.nodeName.toLowerCase() == "select"){
						...
						this.element.trigger("addEvent",["change", el, undefined])
					}
					break;
				case 'scroll':
					if(ev.target.nodeName.toLowerCase() == "select"){
			...
			if(!this.isMouseDown){
				...
				this.scrollTimeout = setTimeout(function(){
				  if(self.scroll != null){
						...
						self.element.trigger("addEvent", ["scroll", self.scroll.target, direction, amount]);
